...
# Поставленные цели в дипломе

- Предпочтение отдавать технологиям в которых меньше опыта
- Креативность оформления отчета
...
- подготовка данных для UI тестов - **GraphQl**. Подумал что маловероятно буду в работе использовать его, скорее всего
  это будет **gRPC**/**RestFull Api**/**SQL**

# Что особенного в проекте

Экспериментально разработал + реализовал для автотестов концепцию смайл(ы) == страница, либо функциональность имеющая интуитивно
понятные границы, подробнее в таблице:

| Функциональность/Страница      | Смайл    |
|--------------------------------|----------|
| Подготовка тестового окружения | ⚙️       |
| Приветствие                    | 🤗       |
| Авторизация                    | 🔒       |
| Регистрация                    | 🔑       |
| Люди                           | 👤 👤 👤 |
...
⚙️/🧹 - в виде исключения добавил для обозначения подготовки/очистки тестового окружения

Пример:

<img src="readme-resources/smile_example.png" width="1000">

❗ подход спорный для больших и сложных проектов

2. Стандартизация вывода тестовых наборов
