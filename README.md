...
- Эксперимента ради применён для заготовки данных **GraphQl**, в реальном продакшн проекте предпочел бы **gRPC** либо *
  *SQL** запросы в зависимости от обстоятельств, в целом проект для меня был экспериментальным.
- Цель - попытаться не стандартно покрыть тестами. В процессе написания предпочтение отдавал технологиям с которыми
  меньше всего опыта в работе.

Например:

- мало опыта с cssSelector - то что надо!
- подготовка данных для UI тестов - **GraphQl**. Подумал что маловероятно буду в работе использовать его, скорее всего
  это будет **gRPC** либо **RestFull Api**. Возможно я ошибся, но не ошибается тот кто ничего не делает)

Экспериментально разработал + реализовал концепцию смайл(ы) == страница, либо функциональность имеющая интуитивно
понятные границы, а подробнее:

| функциональность                                | Смайл    |
|-------------------------------------------------|----------|
| Welcome                                         | 🤗       |
| Авторизация                                     | 🔒       |
| Регистрация                                     | 🔑       |
...
| Profile                                         | 🧑       |
| Travels                                         | 🌎       |
| Абстракция <br/>не привязанная <br/>к чему либо | ❓        |

- считаю данный подход спорной для сложных проектов идеей
...
- GraphiQL интерфейс сервиса rangiffler-gateway доступен по адресу: http://gateway.rangiffler.dc:8080/graphiql (в РФ
  работает с VPN )
...
2. **Если вы не можете подключиться к БД в docker, указывая верные login и password**, то возможно у вас поднята другая
   база на
   том же порту 3306.
